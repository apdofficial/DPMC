...
int Dd::postGCHook(DdManager *dd, const char *str, void *data)
{
    ...
    printLine("GC done! memused: " + to_string(memused) + " / " + to_string(memMax) + " = " +
              to_string((memused + 0.0) / memMax));
    noReordSinceGC = true;
    return 1;
}
...
int Dd::preReorderHook(DdManager *dd, const char *str, void *data)
{
    ...
    retval = fprintf(dd->out, ": from %ld to ... ",
                     strcmp(str, "BDD") == 0 ? Cudd_ReadNodeCount(dd) : Cudd_zddReadNodeCount(dd));
    if (retval == EOF) return (0);
    fflush(dd->out);
    return (1);
...
Float Dd::getNegWt(Int ddVar)
{
    ...
    if (negWt == 1) {
        ...
        if (!isApproximatelyEqual(negWt + posWt, 1.0l, 0.001l)) {
            printLine("literal weights not summing to 1!! :" + to_string(ddVar) + "  " + to_string(posWt) + ":" +
                      to_string(negWt));
            exit(1);
        }
    }
...
Dd Dd::getAbstraction(Map<Int, tuple<Number, Number, bool, Int>> ddVarWts, Float logBound,
                      vector<pair<Int, Dd>> &maximizationStack, bool maximizerFormat, bool substitutionMaximization,
                      Int verboseSolving)
{
    manualReorder();
    if (atomicAbstract) {
        if (ddPackage == CUDD_PACKAGE) {
            ...
            if (weightedCounting) {
                return logCounting ? cuadd.WeightedLogSumExistAbstract(wCube, getNegWt) : cuadd.WeightedExistAbstract(
                        wCube, getNegWt);
            } else {
...
bool Dd::beforeReorder()
{
    ...
    if (ddPackage == SYLVAN_PACKAGE) {
        ...
        if (preUsed > sylvan::nodes->max_size * reordThresh) { // comparing to max_size ensures table has grown fully
            printLine("Used fraction: " + to_string(preUsed) + "/ " + to_string(sylvan::nodes->max_size) + "*" +
                      to_string(reordThresh) + " = " + to_string(preUsed / (sylvan::nodes->max_size * reordThresh)));
            return true;
        } else {
            return false;
        }
        ...
        if (mgr->ReadSlots() < lut) {
            ...
            if (usedFrac > reordThresh) {
                printLine("Used fraction of slots: " + to_string(usedFrac) + "  ReordThresh:" + to_string(reordThresh) +
                          " Expected usage: " + to_string(Cudd_ExpectedUsedSlots(mgr->getManager())));
                return true;
            } else {
                return false;
            }
...
//      sylvan::sylvan_set_reorder_timelimit_sec(1 * swapTime);
        }
...
void Dd::init(string ddPackage_, Int numVars, bool logCounting_, bool atomicAbstract_, bool weightedCounting_,
              bool multiplePrecision_, Int tableRatio, Int initRatio, Int threadCount, Float maxMem,
              Int dynVarOrdering_, Int dotFileIndex_)
{
...
//      sylvan::sylvan_set_reorder_threshold(256);
